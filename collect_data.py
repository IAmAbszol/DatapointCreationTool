...
def rescale(base, desired, coordinates):
    ...
    return [int(coordinates[0] / scale_w), int(coordinates[1] / scale_h), int(coordinates[2] / scale_w),
            int(coordinates[3] / scale_h)]
...
def write_to_file(file, name, coordinates):
    file.write(str(name) + " 1 {} {} {} {}\n".format(int(coordinates[0]), int(coordinates[1]),
...
parse.add_argument("-s", "--sleep", required=True, help="sleep time during while loop, in seconds")
...
random.seed()
while True:
    time.sleep(random.randint(0, int(args["sleep"])))
    # add timer delay
    ...
    for res in resolutions:
        ...
        for x in range(len(regions)):
            ...
            increment_calculation = [(main_region_coordinates[0] - sub_region_coordinates[0]),
                                     (main_region_coordinates[1] - sub_region_coordinates[1]),
                                     (main_region_coordinates[2] - sub_region_coordinates[2]),
                                     (main_region_coordinates[3] - sub_region_coordinates[3])]
            for main_sub_regions in range(1, sub_control, 1):
                distance = [(increment_calculation[0] / main_sub_regions),
                            (increment_calculation[1] / main_sub_regions),
                            (increment_calculation[2] / main_sub_regions),
                            (increment_calculation[3] / main_sub_regions)]
                # now bridge these directions off the icon subregion
                created_main = [sub_region_coordinates[0] + distance[0], sub_region_coordinates[1] + distance[1],
                                sub_region_coordinates[2] + distance[2], sub_region_coordinates[3] + distance[3]]
                # created main compared to sub_region_coordinates will yield the icons image relative to the bounding image.
                cropped_im = im.crop(
                    box=(int(created_main[0]), int(created_main[1]), int(created_main[2]), int(created_main[3])))
                # im = ImageGrab.grab(bbox=(int(created_main[0]), int(created_main[1]), int(created_main[2]), int(created_main[3])))
                cropped_im.save(args["directory"] + "/" + args["directory"] + "-" + str(global_image) + ".png")
                # calculate icons position relative to the picture itself aka bounding box created, fixed for exceeding picture bounds. This was a possible cause for tensorflows irrationality towards updated records
                print(sub_region_coordinates)
                adjusted_sub = [max(int(sub_region_coordinates[0] - created_main[0]), 0),
                                max(int(sub_region_coordinates[1] - created_main[1]), 0),
                                min(int(created_main[2] - created_main[0]),
                                    int(sub_region_coordinates[2] - sub_region_coordinates[0])),
                                min(int(created_main[3] - created_main[1]),
                                    int(sub_region_coordinates[3] - sub_region_coordinates[1]))]
                print(str(global_image) + ".png - " + str(main_sub_regions) + ": " + str(adjusted_sub[0]) + ", " + str(
                    adjusted_sub[1]) + ", " + str(adjusted_sub[2]) + ", and " + str(adjusted_sub[3]))
                write_to_file(file, args["directory"] + "-" + str(global_image) + ".png", adjusted_sub)
                global_image += 1
